...
class AiChatBubble extends Component
{
    ...
    public function sendMessage(): void
    {
        ...
        $apiKey = Setting::get('openai_api_key', ''); // Déjà déchiffrée par le modèle Setting
    ...
    protected function buildContextualPrompt(string $userMessage, array $context): string
    {
        ...
        $recentHistory = array_slice($this->chatHistory, -4); // Derniers 4 messages pour économiser les tokens
        ...
        $prompt .= "Réponds de manière claire et concise. Si tu génères du SQL, assure-toi qu'il soit compatible MySQL et utilise les noms de tables/colonnes exacts du contexte.";
    ...
    {
        $this->chatHistory = session('ai_chat_bubble_history', []);
    }

    protected function saveChatHistory(): void
    {
        session(['ai_chat_bubble_history' => $this->chatHistory]);
    }
